...
const message = {
    ...
    xpack: {
        ...
        vm: {
            ...
            forceDeleteConfirm:
                'libvirt のクリーンアップに失敗した場合、強制削除によりホスト上にまだ存在または実行中の仮想マシンが隠れる可能性があります。続行しますか？',
            deleteNameHelper: '" と入力してこの仮想マシンを削除',
            isoSource: 'ISO ソース',
            isoTargetDir: 'ISO 保存パス',
            isoTargetDirHelper:
                'サーバーで選択した ISO とローカルアップロードした ISO はこのディレクトリに保存されます。',
            isoTargetDirAvailable: 'ISO 保存パスは利用可能です。',
            isoTargetDirUnavailable: 'ISO 保存パスは利用可能な仮想マシンストレージプール内である必要があります。',
            checkPath: '確認',
            isoSize: 'ISO サイズ',
            dir: 'ローカルディレクトリ',
        },
