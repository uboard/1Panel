...
const message = {
    commons: {
        ...
        login: {
            ...
            agreeContent:
                'Для лучшей защиты ваших законных прав и интересов, пожалуйста, прочитайте и согласитесь со следующим соглашением &laquo; ',
    ...
    aiTools: {
        agents: {
            ...
            verifyModel: 'Модель проверки',
            verifyModelHelper:
                'Выберите модель, доступную этому аккаунту. При сохранении будет отправлен минимальный запрос для проверки подключения.',
            verifyModelRequired: 'Выберите модель проверки',
            discoverModels: 'Получить модели',
            automaticModelDiscovery: 'Получить автоматически',
        ...
        aiProxy: {
            ...
            rebuildVectorsConfirm:
                'Пересоздание векторов снова отправит образцы Smart Route и проверки контента в сервис Embedding. Продолжить?',
            ...
            bestSampleSimilarity: 'Макс. сходство образца',
            source: 'Источник',
        ...
        skillsHub: {
            ...
            metadataOverwriteConfirmWithVersions:
                'Версия в пакете Skill отличается от текущего ввода. Версия в пакете: {packageVersion}; введенная версия: {inputVersion}. Использовать текущий ввод?',
        ...
        mcp: {
            ...
            gatewayArgsHelper:
                'Аргументы добавляются к команде supergateway. Параметры транспорта, порта, пути и протокола, управляемые 1Panel, изменить нельзя',
    ...
    xpack: {
        ...
        logExport: {
            title: 'Экспорт журналов',
            records: 'История экспорта',
            retentionDays: 'Срок хранения истории экспорта',
            timeRange: 'Диапазон времени',
            timeRequired: 'Выберите диапазон времени экспорта',
            websiteRequired: 'Выберите веб-сайт',
            blackStatus: 'Статус черного списка',
            black: 'Заблокирован',
            notBlack: 'Не заблокирован',
            fuzzy: 'Нечеткий поиск',
            regex: 'Регулярное выражение',
            invalidRegex: 'Введите корректное выражение RE2 длиной до 256 символов',
            created: 'Задача экспорта создана',
            fileName: 'Имя файла',
            fileSize: 'Размер файла',
            expiresAt: 'Срок действия',
            cancelConfirm: 'Отменить эту задачу экспорта?',
            deleteConfirm: 'Удалить эту запись экспорта и файл?',
        },
        waf: {
            name: 'WAF',
            ...
            geoHelper:
                'Объединяет правила регионов и провинций одним переключателем. Сначала применяются правила регионов, затем правила провинций',
            regionPolicy: 'Правила регионов',
            provincePolicy: 'Правила провинций',
            ...
            deny: 'Запрещено',
            allow: 'Разрешить',
            allowOnly: 'Разрешить только выбранное',
            field: 'Объект',
            pattern: 'Условие',
            ...
            hostSearchHelper: 'Введите Host для нечеткого поиска',
            toCreate: 'Создать',
            ...
            originProtect: 'Защита источника CDN',
            attacklog: 'Запись перехвата',
            unknownWebsite: 'Неавторизованный доступ к домену',
            geoRuleEmpty: 'Выберите хотя бы один регион или провинцию',
            ...
            spiderIpHelper:
                'Если включено, разрешены только выбранные поисковые роботы; если выключено, доступ всех роботов блокируется.',
            spiderIp: 'Пул IP-адресов поисковых роботов',
            spider: 'Поисковые роботы',
            spiderAllowlist: 'Разрешенные поисковые роботы',
            spiderAllowlistEmpty:
                'Поисковые роботы не выбраны. После сохранения доступ всех роботов будет заблокирован.',
            ...
            executionStrategy: 'Стратегия выполнения',
            detectionStrength: 'Уровень обнаружения',
            protectionMode: 'Режим защиты',
            observationMode: 'Режим наблюдения',
            standardMode: 'Стандартный режим',
            observationModeHelper: 'Обнаруживает и записывает запросы, не блокируя их',
        ...
        monitor: {
            ...
            realIPRecognition: 'Определение реального IP',
            ...
            originProtection: 'Защита исходного сервера',
            originProtectionHelper:
                'К источнику могут подключаться только узлы CDN из выбранных групп IP. Определение реального IP посетителя включается отдельно.',
            originProtectionIPGroups: 'Группы IP источника CDN',
            originProtectionIPGroupsPlaceholder: 'Выберите доверенные группы IP источника CDN',
            originProtectionWarning:
                'Ошибка конфигурации может сделать сайт недоступным. Убедитесь, что добавлены все адреса обратного подключения CDN.',
            originProtectionEnableConfirm:
                'Подключения вне выбранных групп IP будут считаться прямым доступом к источнику и блокироваться. Продолжить?',
        ...
        vm: {
            ...
            healthCheckResult: {
                ...
                fail: 'Ошибка',
                normal: 'Норма',
                fixCommand: 'Команда исправления',
                technicalDetail: 'Технические сведения',
                check: {
                    vmName: 'Имя виртуальной машины',
                    libvirtConnection: 'Подключение к Libvirt',
                    vmDefinition: 'Определение виртуальной машины',
                    domainXML: 'XML виртуальной машины',
                    architecture: 'Архитектура',
                    virtualizationType: 'Тип виртуализации',
                    machineType: 'Тип машины',
                    efiCapability: 'Поддержка EFI',
                    qemuEmulator: 'Эмулятор QEMU',
                    bootOrder: 'Порядок загрузки',
                    arm64Firmware: 'Прошивка ARM64',
                    arm64Features: 'Функции ARM64',
                    arm64Video: 'Видеоустройство ARM64',
                    arm64FirmwareFile: 'Файл прошивки ARM64',
                    arm64CDROM: 'CD-ROM ARM64',
                    blockDevice: 'Блочное устройство',
                    isoArchitecture: 'Архитектура ISO',
                    network: 'Сеть',
                    qemuLog: 'Журнал QEMU',
                    arm64RuntimeFirmware: 'Прошивка ARM64 во время выполнения',
                },
                reason: {
                    invalidName: 'Недопустимое имя виртуальной машины.',
                    connectionFailed: 'Не удалось подключиться к {uri}.',
                    vmNotFound: 'Виртуальная машина {name} не найдена.',
                    xmlReadFailed: 'Не удалось прочитать XML виртуальной машины.',
                    liveXMLFallback:
                        'Не удалось прочитать постоянную конфигурацию; используется текущая конфигурация запущенной машины.',
                    xmlParseFailed: 'Не удалось разобрать XML виртуальной машины.',
                    stateReadFailed:
                        'Не удалось определить состояние виртуальной машины. Проверка прошивки во время выполнения пропущена.',
                    guestArchMissing: 'В XML отсутствует архитектура гостевой системы.',
                    archMismatch:
                        'Архитектура хоста — {hostArch}, а гостевой системы — {guestArch}. Выполнение между разными архитектурами не поддерживается.',
                    nonKVMType: 'Тип виртуализации — {type}, а не KVM по умолчанию.',
                    archOrMachineMissing: 'В XML отсутствует архитектура или тип машины.',
                    machineUnsupported: 'Хост не поддерживает {arch}/{machine}.',
                    emulatorNotSpecified: 'Эмулятор не указан в XML; Libvirt выберет его автоматически.',
                    fileNotFound: 'Файл не найден: {path}',
                    fileNotExecutable: 'Файл не является исполняемым: {path}',
                    bootDeviceMissing: 'В XML не определено загрузочное устройство.',
                    diskNotFirst: 'Диск не является первым загрузочным устройством. Текущий порядок: {order}',
                    cdromNotInBootOrder: 'CD-ROM подключён, но отсутствует в порядке загрузки.',
                    firmwareNotConfigured:
                        'EFI или Loader не настроены, поэтому гостевая система может не инициализировать экран или не загрузиться.',
                    acpiCustom: 'ACPI настроен. Убедитесь, что гостевой системе действительно нужна эта настройка.',
                    acpiRequiresUEFI: 'ACPI настроен без UEFI, что вызывает ошибку ACPI requires UEFI.',
                    apicUnsupported: 'Настроен x86 APIC, который недопустим для ARM64.',
                    gicNotDeclared: 'GIC отсутствует в постоянном XML; Libvirt может добавить его во время выполнения.',
                    videoNotConfigured: 'В XML не настроено видеоустройство.',
                    videoNotVirtio: 'Тип видеоустройства — {type}; рекомендуется Virtio.',
                    firmwareFileMissing: 'Файл прошивки не существует или не является обычным файлом: {path}',
                    firmwareNotAvailable: 'Libvirt не сообщает о поддержке EFI, и прошивка ARM64 UEFI не найдена.',
                    cdromBusNotSCSI: 'CD-ROM использует шину {bus}; для ARM64 требуется SCSI.',
                    cdromNotOnVirtioSCSI: 'CD-ROM не подключён к контроллеру Virtio SCSI.',
                    diskSourceMissing: 'Для {device} {target} не настроен исходный файл.',
                    sourceNotFound: 'Исходный файл для {device} {target} не существует: {path}',
                    sourceIsDirectory: 'Исходный путь для {device} {target} является каталогом: {path}',
                    systemDiskNotFound: 'Системный диск не обнаружен.',
                    isoArchMismatch:
                        'К гостевой системе {guestArch} подключён образ, предназначенный, вероятно, для {imageArch}: {path}',
                    networkReadFailed: 'Не удалось прочитать сеть {name}.',
                    networkStateReadFailed: 'Не удалось прочитать состояние сети {name}.',
                    networkInactive: 'Сеть {name} существует, но не активна.',
                    logUnavailable: 'Журнал запуска для анализа отсутствует.',
                    logErrorsFound: 'В последней записи запуска обнаружены распространённые ошибки.',
                    vmNotRunning:
                        'Виртуальная машина не запущена, поэтому проверка аргументов QEMU во время выполнения пропущена.',
                    runtimeFirmwareMissing: 'В журнале QEMU не найдены записи pflash, AAVMF или QEMU_EFI.',
                },
            },
            ...
            cpuLimitHelper: 'Доступно CPU сейчас: {0} ядер',
            cpuLimitExceeded: 'CPU превышает лимит. Текущий максимум: {0} ядер.',
            memoryLimitHelper: 'Доступная для выделения память: {0} (хост должен зарезервировать {1} памяти)',
            memoryLimitExceeded: 'Память превышает лимит. Текущий максимум для выделения: {0}.',
            storageLimitHelper: 'Текущее свободное место в целевом пуле хранения: {0}',
            storageLimitExceeded: 'Ёмкость диска превышает лимит. Текущий максимум: {0}.',
            iso: 'ISO',
            ...
            deleteTemplateConfirm: 'Удалить шаблон {0}. Эту операцию нельзя отменить. Продолжить?',
            orphanClean: 'Очистка неиспользуемых данных',
            orphanScan: 'Повторить сканирование',
            orphanCleanHelper:
                'Отображаются только файлы-кандидаты, на которые не ссылаются виртуальные машины, снимки или зарегистрированные шаблоны. Перед очисткой убедитесь, что файл больше не используется: операцию нельзя отменить.',
            orphanSelected: 'Выбрано файлов: {0}, всего {1}',
            orphanSize: 'Размер',
            orphanCleanConfirm: 'Очистить выбранные файлы: {0} ({1})? Эту операцию нельзя отменить.',
            orphanCleanSuccess: 'Очищено файлов: {0}, освобождено: {1}.',
            orphanType: {
                orphan_domain_disk: 'Неиспользуемый диск',
                orphan_snapshot: 'Неиспользуемый снимок',
            ...
            bootOrder: 'Порядок загрузки',
            bootDiskFirst: 'Сначала диск (HD → CD-ROM)',
            bootIsoFirst: 'Сначала ISO (CD-ROM → HD)',
            bootOrderConfirm:
                'Изменить порядок загрузки на {0}? При следующем запуске ВМ проверит в этом порядке подключённые загрузочные устройства.',
            ...
            downloadFromServer: 'Удалённая загрузка',
            ...
            deleteSourceFileHelper:
                'Если отмечено, исходный файл, на который ссылается эта запись ISO, также будет удалён без возможности восстановления.',
