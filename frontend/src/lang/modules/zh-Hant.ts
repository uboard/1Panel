...
const message = {
    ...
    xpack: {
        ...
        vm: {
            ...
            helperNotStart: '虛擬機執行環境未就緒，請處理主機虛擬化服務後再使用虛擬機功能。',
            cpu: 'CPU',
            memory: '記憶體',
            disk: '磁碟',
            memoryLimitHelper: '最大可分配記憶體：{0}',
            memoryLimitExceeded: 'VM 記憶體分配將超過主機可分配記憶體，請降低記憶體後重試。',
            ...
            diskSize: '磁碟容量',
            diskPath: '磁碟路徑',
            diskPathHelper: '可留空，系統將在所選儲存池目錄下自動建立 虛擬機名稱.qcow2；如填寫，必須是絕對路徑。',
            imagePath: '映像路徑',
            imagePathHelper: '可留空；如填寫，需為已存在的 qcow2 映像絕對路徑。',
            isoPath: 'ISO 路徑',
            isoPathHelper: '可留空；如填寫，需為已存在的 ISO 絕對路徑。',
            storagePool: '儲存池',
            storagePoolHelper: '預設使用 default 儲存池，用於建立和管理虛擬機磁碟卷。',
            network: '網路',
            networkHelper: '預設使用 default NAT 網路；也可以選擇或輸入已存在的 libvirt 網路名稱。',
            bridgeName: '橋接名稱',
            natNetworkHelper:
                'NAT 會建立 libvirt 虛擬網路供虛擬機出網使用，內部橋接名稱會自動產生，不需要選擇主機橋接網卡。',
            bridgeNetworkHelper: 'Bridge 會將虛擬機接入主機既有的 Linux bridge，使虛擬機加入主機所在的二層網路。',
            bridgeNameHelper:
            ...
            dhcpEnd: 'DHCP 結束位址',
            bootOrder: '啟動項',
            bootOrderHelper: 'HD 表示優先從磁碟啟動，磁碟不可啟動時再嘗試 ISO；CD-ROM 表示優先從 ISO 啟動。',
            ...
            forceDeleteConfirm: '如果 libvirt 清理失敗，強制刪除可能隱藏宿主機上仍存在或仍在執行的虛擬機，是否繼續？',
            deleteNameHelper: '" 刪除此虛擬機',
            isoSource: 'ISO 來源',
            isoTargetDir: 'ISO 儲存路徑',
            isoTargetDirHelper: '伺服器選擇和本機上傳的 ISO 檔案都會儲存到該目錄。',
            isoTargetDirAvailable: 'ISO 儲存路徑可用。',
            isoTargetDirUnavailable: 'ISO 儲存路徑必須位於可用的虛擬機儲存池內。',
            checkPath: '檢查',
            isoSize: 'ISO 大小',
            dir: '本機目錄',
        },
