...
const message = {
    ...
    xpack: {
        ...
        vm: {
            ...
            helperNotStart: '가상 머신 실행 환경이 준비되지 않았습니다. 호스트 가상화 서비스를 처리한 후 사용하세요.',
            cpu: 'CPU',
            memory: '메모리',
            disk: '디스크',
            memoryLimitHelper: '최대 할당 가능 메모리: {0}',
            memoryLimitExceeded:
                'VM 메모리 할당이 호스트의 할당 가능 메모리를 초과합니다. 메모리를 줄인 후 다시 시도하세요.',
            ...
            diskPath: '디스크 경로',
            diskPathHelper:
                '비워둘 수 있습니다. 시스템은 선택한 스토리지 풀 디렉터리에 가상머신이름.qcow2를 자동 생성합니다. 입력하는 경우 절대 경로여야 합니다.',
            imagePath: '이미지 경로',
            imagePathHelper: '비워둘 수 있습니다. 입력하는 경우 기존 qcow2 이미지의 절대 경로여야 합니다.',
            isoPath: 'ISO 경로',
            isoPathHelper: '비워둘 수 있습니다. 입력하는 경우 기존 ISO의 절대 경로여야 합니다.',
            storagePool: '스토리지 풀',
            ...
            consoleConnecting: 'VNC 콘솔에 연결 중...',
            ...
            forceDeleteConfirm:
                'libvirt 정리에 실패하면 강제 삭제로 인해 호스트에 아직 존재하거나 실행 중인 VM이 숨겨질 수 있습니다. 계속하시겠습니까?',
            deleteNameHelper: '" 입력 시 이 가상 머신 삭제',
            isoSource: 'ISO 출처',
            isoTargetDir: 'ISO 저장 경로',
            isoTargetDirHelper: '서버에서 선택한 ISO와 로컬로 업로드한 ISO 파일은 이 디렉터리에 저장됩니다.',
            isoTargetDirAvailable: 'ISO 저장 경로를 사용할 수 있습니다.',
            isoTargetDirUnavailable: 'ISO 저장 경로는 사용 가능한 VM 스토리지 풀 내에 있어야 합니다.',
            checkPath: '확인',
            isoSize: 'ISO 크기',
            dir: '로컬 디렉터리',
        },
