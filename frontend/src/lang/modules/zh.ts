    ...
    xpack: {
        ...
        vm: {
            ...
            dependencyPurposeMap: {
                kvm: '提供硬件虚拟化能力，是虚拟机加速运行的基础。',
                libvirt: '提供虚拟机管理服务，负责虚拟机生命周期和资源调度。',
                storage: '提供默认存储目录或存储池，用于保存虚拟机磁盘文件。',
            },
            dependencyNotReady: '虚拟机依赖检测未通过，请处理依赖后再使用虚拟机功能。',
            helperUnavailable: '虚拟机能力初始化失败，请检查 Core 日志或重新升级后再使用虚拟机功能。',
            helperNotStart: '虚拟机运行环境未就绪，请处理宿主机虚拟化服务后再使用虚拟机功能。',
            cpu: 'CPU',
            memory: '内存',
            disk: '磁盘',
            memoryLimitHelper: '最大可分配内存：{0}',
            memoryLimitExceeded: 'VM 内存分配将超过宿主机可分配内存，请降低内存后重试。',
            ...
            diskSize: '磁盘容量',
            diskPath: '磁盘路径',
            diskPathHelper: '可留空，系统将在所选存储池目录下自动创建 虚拟机名称.qcow2；如填写，必须是绝对路径。',
            imagePath: '镜像路径',
            imagePathHelper: '可留空；如填写，需为已存在的 qcow2 镜像绝对路径。',
            isoPath: 'ISO 路径',
            isoPathHelper: '可留空；如填写，需为已存在的 ISO 绝对路径。',
            storagePool: '存储池',
            storagePoolHelper: '默认使用 default 存储池，用于创建和管理虚拟机磁盘卷。',
            network: '网络',
            networkHelper: '默认使用 default NAT 网络；也可以选择或输入已存在的 libvirt 网络名称。',
            bridgeName: '桥接名称',
            natNetworkHelper:
            ...
            dhcpEnd: 'DHCP 结束地址',
            bootOrder: '启动项',
            bootOrderHelper: 'HD 表示优先从磁盘启动，磁盘不可启动时再尝试 ISO；CD-ROM 表示优先从 ISO 启动。',
            ...
            forceDeleteConfirm: '如果 libvirt 清理失败，强制删除可能隐藏宿主机上仍存在或仍在运行的虚拟机，是否继续？',
            deleteNameHelper: '" 删除此虚拟机',
            isoSource: 'ISO 来源',
            isoTargetDir: 'ISO 保存路径',
            isoTargetDirHelper: '服务器选择和本地上传的 ISO 文件都会保存到该目录。',
            isoTargetDirAvailable: 'ISO 保存路径可用。',
            isoTargetDirUnavailable: 'ISO 保存路径必须位于可用的虚拟机存储池内。',
            checkPath: '检查',
            isoSize: 'ISO 大小',
            dir: '本地目录',
        },
